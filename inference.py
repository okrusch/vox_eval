...
def main(args, device):
    ...
    for i, loaded_data in enumerate(test_loader):
        rt = dt.datetime.now() # rtf start point
        ...
        if args.vocoder == "vocos":
            ...
            # calc rtf
            rt = (dt.datetime.now() - rt).total_seconds()
            rtf =  rt * 24000 / wav.shape[-1]
    ...
    # write rtf dict to file
    with open(args.rtf_path, 'w') as f:
        json.dump(rtf_dict, f)
...
def prepare_models(args, device):
    ...
    ckpt = torch.load(args.nar_ckpt, map_location=device, weights_only=False)
    ...
    state_dict = torch.load(f"{hp.vocos_path}/pytorch_model.bin", map_location="cpu", weights_only=False)  
    encodec_parameters = {
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rtf_path',
                        type=str,
                        help='where to store a json file with rtf values')            
